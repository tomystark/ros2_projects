...
public:
    NumberCounterNode() : Node("number_counter"), counter_(0)
    {
        ...
        RCLCPP_INFO(this->get_logger(),"reset_counter service has started");    
    }
...
private:
    ...
    void callbackResetCounter(const example_interfaces::srv::SetBool::Request::SharedPtr request,
                        const example_interfaces::srv::SetBool::Response::SharedPtr response)
                        {
                            if (request->data)
                                {
                                    counter_publisher_ = 0;
                                    response->success = true;
                                    response->message = "Counter has been restarted";
                                    RCLCPP_INFO(this->get_logger(), "Counter has been restarted");
                                }
    }
